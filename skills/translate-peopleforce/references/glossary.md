...
**How a flag gets here.** The source carries a per-cell status; the three statuses that
mean *somebody has doubted this* render the 🚩, and the rest render quiet. So a flag sits
on every cell somebody has doubted, and no flag on cells that are either settled or simply
unexamined — most of the table is unexamined. The full status set and what each renders is
documented beside the renderer, in `translation-corpus/README.md`.
...
| EN | ES | PL | UK | Notes |
|---|---|---|---|---|
| **employee** | empleado / colaborador (mixed-contract populations (dependent + monotributista/freelance), per-seat pricing lines, and engagement or culture copy) | pracownik | співробітник | **The split is scope and register, not law.** `empleado` is the written default and the product agrees. `colaborador` is the sales floor's word, and it is the right one where the population is mixed-contract — it covers people `empleado` excludes — and in per-seat pricing lines and culture copy. |
...
| **salary bands (the pay structure — the band system itself) / salary range (the published number on a job ad)** | bandas salariales (the pay structure) / rango salarial (the published number on a job ad) 🚩 | widełki płacowe | зарплатні діапазони | **en:** Owner ruling. · **en:** Keep the two senses apart · **pl:** **The source's own `widełki stanowiskowe` has 0 corpus hits** — translate it to this, do not mirror it · **uk:** Owner ruling. 3 corpus hits; `діапазон зарплат` has 6 and is the commoner shape, so 🚩 a native could reasonably prefer that. Product uses `Діапазон оплати праці` (`b2b-uk-UA.yml:990`) and `Діапазон базової компенсації` for the field labels — use the product's form when the line points at a screen |
...
| **preboarding** | preboarding | preboarding | пребординг | **es:** Kept latin in es and pl, and it inflects: pl `preboardingu`. The step before day one, distinct from onboarding — uk reps say `форма прибордингова` for its form. Owner ruling 2026-08-12: the concept key and the en/es/pl cells are `preboarding`, replacing the earlier `pre-onboarding` outright — the older form is no longer a house form and is not kept as a variant · **uk:** Owner ruling 2026-08-12: uk **transliterates** where es and pl keep latin. `пребординг` declines (`пребордингу`), which the latin string cannot, and it is attested in Ukrainian HR writing outside our corpus — work.ua, itexpert.work, and a 2024 *БІЗНЕСІНФОРМ* paper — so it is an existing borrowing rather than a coinage. First shipped in the onboarding-automation lead magnet |
...
| **stakeholders** | interesados 🚩 | interesariusze | стейкхолдери / зацікавлені сторони 🚩 | **pl:** No competing form attested · **uk:** Owner ruling: asked for the row following `uk.md` §4's transliteration pattern — but **§4 is itself marked "derived, not decided"**, so this inherits that doubt. A native should choose between this and `зацікавлені сторони` |
