...
| EN | ES | PL | UK | Notes |
|---|---|---|---|---|
| **employee** | empleado / colaborador (the person in the system whatever their contract form (mixed dependent + monotributista/freelance populations), per-seat pricing lines, and engagement or culture copy — where published articles skew it 62% culture) | pracownik | співробітник | **The split is scope and register, not law.** `empleado` is the written default — 1,898 hits/145 articles against `colaborador` 189, and the product agrees (`b2b-es-ES.yml:1187`, `attributes-es-ES.yml:401`). `colaborador` is the sales floor's word (1,686/109 against 1,313/113 in speech) and its job is **coverage**: a rep gives the platform's own definition, `entendemos a todos como colaboradores, no importa si es en relación de dependencia, si es monotributista` — it includes people `empleado` excludes. Not a legal split: legal/contract context is 8% of `colaborador` hits against 7% of `empleado`. pl and uk share a lexeme with `staff member` deliberately — see those cells |
...
| **manager** | líder / gerente (where the org-chart level is the point rather than the person's role) | manager (the head form — decline it: `managerowi`, `managera`, `managerów`) / menedżer (the Polish spelling, where a line wants one instead of the latin loan) / przełożony (the line-manager RELATIONSHIP — who someone reports to, rather than the job title) / kierownik (a team or unit head, where Polish would name the post rather than borrow) | менеджер | **es:** **It splits by language, which is why it was never a frozen loanword.** es: published copy prefers `líder` (216 hits/70 articles) over English `manager` (174/50) and `gerente` (92/25) — reps say the English word constantly (922/117) but that is call register. pl: published copy says `manager` 220/56 against `menedżer` 63/21, which backs what the preamble told translators to do provisionally; in speech the spelling `menadżer` dominates (481/70) and is the non-standard form, so do not follow speech here. uk: `менеджер` in Cyrillic, 542 hits/125 articles, standalone and inside compounds · **pl:** CONFIRMED 2026-08-11 by measuring what the earlier note never asked: the bare latin loan is what our Polish copy actually writes, **625 hits across 57 documents**, against `menedżer` 177/21 and `menadżer` 6/2. So it beats the Polish spelling roughly 3:1 on both hits and document spread and keeps the head. It must decline — an undeclined loan is the defect, not the loan · **pl:** SETTLED 2026-08-11, correcting two claims this row carried. (1) **`menadżer` is not "never correct"** — PWN records both; *Wielki słownik poprawnej polszczyzny* marks it `rzadkie` (rare) and RJP PAN recommends `menedżer`. Rare is not wrong. (2) The 29:1 figure quoted here settles **which Polish spelling**, not whether to spell it in Polish — the latin loan beats both, so the head form is unchanged. Where a line wants the Polish spelling write `menedżer`: 177 hits/21 docs, inflecting freely (`menedżerów` 76, `menedżera` 24). `menadżer` is rep speech, 224r:21r, while prospects prefer `menedżer` 104p:84p · **pl:** Use it where the sentence is about the reporting line |
...
| **screening** | selección / filtrar / primer filtro (the ACT of screening, as a verb or as the named step) / preselección (the narrowing pass specifically — filtering a candidate pool down before interviews) | screening | скринінг / відбір (where the word has to inflect or agree, and in formal register) | SETTLED 2026-08-11 on the written corpus: `selección` is **not** too narrow — the flag rested on call data alone. The open piece was never the head form but a **part-of-speech split**: `selección` names the stage, `filtrar` performs it, `preselección` is the narrowed pool. `cribado` DROPPED at 3 hits/1 doc. uk: `скринінг` (84/11) and `відбір` (39/9) both house forms — prefer `відбір` where the word must inflect or the register is formal |
| **onboarding** | onboarding (the lifecycle stage, the product module, the named process) / incorporación (general meaning — flowing prose, and the experience of joining) | onboarding | онбординг | **es:** SETTLED against our own es copy 2026-08-11, and the corpus draws the line exactly where the owner did. `onboarding` is what our marketing writes for the **stage and the system** — `Onboarding y baja` as the module pair, `proceso de onboarding`, `onboarding-offboarding`, and the feature-list heading — 156 windows. `incorporación` is the **general-prose** form (row below), 114 windows. The two are not competing: one page carries both, anchor text `onboarding de empleados` on the slug `incorporacion-y-adaptacion-de-los-empleados`. uk confirmed against the localized help centre · **es:** The verb `incorporar` is live too (`incorpora fácilmente a nuevos empleados`). Use this for the general sense and `onboarding` for the stage or the system |
| **onboarding process** | proceso de incorporación 🚩 | proces onboardingu 🚩 | процес онбордингу 🚩 | Prose form — same 🚩 as onboarding |
| **induction** | inducción | — | — | MEASURED 2026-08-11 and it is far narrower than this row implied: **2 windows in the whole es corpus, both `plantillas de inducción`.** So it is not a general-purpose alternative to `incorporación` and never the word for the process as a whole — the earlier flag treated it as a rival to the `onboarding` gloss, which the numbers do not support (`incorporación` 114, `onboarding` 156). Use it for the induction artefact; for anything wider take the `onboarding` row |
...
| **turnover** | rotación | rotacja | плинність кадрів | **es:** uk: the full phrase, not кадрів alone · **uk:** **Write the full collocation in copy**; speech drops the modifier to bare `плинність` (11r/0p), which is register rather than a rival — recognise it coming in, restore the modifier going out. `текучість` is a Russian-leaning spoken variant: do not print it. This cell survived a native escalation that had to be withdrawn — the term was right, the sentence around it was wrong |
...
| **time off** | tiempo libre (the request and the notifications around it) / ausencia (the record, the policy and the calendar view) / licencia (the entitlement in running prose — see the leave row) | nieobecność (the record, the policy and the calendar view) / wolne / czas wolny / dni wolne (the thing an employee takes or asks for, in prose) | відсутність (the record, the policy and the calendar view) / вихідний / день відпустки (the day itself, and the request for one, in prose) | **es:** **Three roles, not three synonyms**, and every language carries the same split. The **request** is `tiempo libre` — what an employee takes, so the verb goes with it. The **record, policy and calendar** sense is `ausencia` (`Ausencias de hoy`, `política de ausencias`). The **entitlement** is `permiso`/`licencia` — see the `leave` row. Match the role before you pick the word. pl and uk split the same way: record is `nieobecność` / `відсутність`, request is `wolne` / `вихідний`, and the verb follows the request form — `wziąć wolne`, `взяти вихідний`. Full construction maps are in each language's §2. **KEEP THIS SPLIT even though the product collapses it** — ruled 2026-08-11 on our own published copy, which already makes the distinction the UI lost and does not treat the two as swappable: pl `urlop*` 696 hits/46 docs against `nieobecność*` 203/29, uk `відпустк*` 343/46 against the record sense of `відсутн*`, whose collocations are plainly systemic (`запити на відсутність`, `відстеження відсутності співробітників`). Following the collapse would mean rewriting copy that currently reads correctly. The two rules do not actually conflict: **where a line names the screen, the product's own string wins** (`references/product-screens.md`); everywhere else the roles above apply · **pl:** Write the verb phrase: `wnioskować o czas wolny`, `wziąć wolne`, `dni wolne`. `wziąć wolne` is spoken (0 written hits), so prose prefers `wnioskować o` + noun. Full map in pl.md §2 · **uk:** Write `взяти вихідний`, `оформити день відпустки`, or name the day. Full map in uk.md §2 |
| **leave** | permiso / licencia (running prose, and any LatAm-facing copy) | urlop | відпустка | es `Permiso` confirmed against the product locales. **In prose write `licencia`** — 683 hits/104 calls, the spoken default for both the entitlement and the request (`licencia por enfermedad`, `solicitud de licencia`) — because `permiso` collides: about a third of its 584 hits are access permissions (`roles y permisos`). Keep `permiso` for the product string, `licencia` everywhere else · **es:** From the corpus: `licencia` 683 hits/104 calls is the spoken default for the entitlement and the request (`licencia por enfermedad`, `solicitud de licencia`). Keep `permiso` for the product string, but note the collision it carries: of 584 `permiso` hits, roughly a third are access permissions (`roles y permisos`), so in prose `licencia` is the unambiguous choice |
| **payroll** | nómina | płace | нарахування заробітної плати | `nómina` is the default; `liquidación de sueldos` is dead in every oracle, so route around it. One hazard inside `nómina` itself: a minority of its uses name the **employee roster** rather than the pay function (`la nómina que van a tener cargada`) — where the source means the list of people, write `directorio` or `listado de colaboradores` and keep `nómina` for pay. Qualify it if a CA/PE piece needs it |
| **payroll changes** | novedades | — | — | What gets sent to payroll or accounting each period |
| **payslip** | recibo de sueldo | odcinek wypłaty / pasek wynagrodzeń / pasek płacowy / pasek wypłaty | розрахунковий лист 🚩 | es: distinct from the payroll-run term · **pl:** **All four name the same document and are interchangeable** — checked sense by sense 2026-08-11, none splits off a distinct meaning. Every attestation is an employee viewing, downloading or signing their payslip. Written: `odcinek wypłaty` 27/4 docs, `pasek wynagrodzeń` 21/3, `pasek płacowy` 9/3, `pasek wypłaty` 2/1. Speech inverts it — `pasek płacowy` 6r/**14p**, `odcinek` 0 — so **prospects say `pasek płacowy`**; prefer it in prospect-facing copy. Only nuance: `odcinek` (stub) reads more administrative than `pasek` (slip). The old cell was bare `pasek wypłaty`, weakest of the four in both registers |
| **salary** | salario / sueldo / remuneración | wynagrodzenie | зарплата (marketing, product and prose copy) / заробітна плата (statutory or contractual text) | 2026-08-11: **synonyms chosen by context, all three live.** `salario` 125 hits/26 docs is the neutral default and the one to reach for; `sueldo` 42/8 is the everyday word for what a person is paid and is what Río de la Plata speech uses (and it heads the compounds — `recibo de sueldo`, `planillas de sueldo`); `remuneración` 33/9 is the formal or contractual register. None is wrong for the concept, so pick by the sentence and hold it across a piece reads legal in marketing copy. Not examined for pl or es |
| **compensation** | compensación | wynagrodzenie | компенсація |  |
| **competency** | competencia | kompetencja | компетенція |  |
| **employee benefits** | beneficios 🚩 | świadczenia / benefity | бенефіти / пільги | **es:** pl: świadczenia, not korzyści, which means advantages · **pl:** Speech cannot arbitrate and does not need to: `benefity` 49r/22p against `świadczenia` 32r/19p, each across 30 calls, with both sides using both words. `świadczenia` leans formal or statutory, `benefity` leans to the package a company advertises, but they overlap freely and neither is a defect. Pick one and hold it across a piece. 🚩 the product's own pl string for this screen is unverified — no locale access from this repo — so match the screen where a line names it · **uk:** Speech gives no signal at all, 2r/0p each way. Pick one and hold it across a piece. 🚩 the product's own uk string for this screen is unverified — no locale access from this repo |
...
| **employee file** | legajo | teczka pracownika | картка співробітника | es: `legajo` is the market term; the product says `perfil del empleado`, so match the product where the line names that screen. **uk is `картка співробітника`, not `профіль`** — different product objects, and reps say `картка` 59 times against 4 |
| **tenure** | permanencia 🚩 | staż pracy | стаж роботи | es: antigüedad in Argentine and Uruguayan copy; permanencia elsewhere |
| **headcount** | dotación | liczba pracowników | кількість співробітників | From the corpus: attested 21 hits/14 calls, so the flag comes off. |
...
| **overtime** | horas extras | nadgodziny | понаднормова робота / понаднормові | **uk:** From the product locale: authoritative: the product says `Понаднормово` and `понаднормова робота` in **52 places** across `crowdin/uk-UA/` (`application-uk-UA.yml:553`, `:1300`, `:189-190`, errors file) and **never** `надурочні`. Elliptical `понаднормові` is fine as a standalone noun in a list · **pl:** `godziny nadliczbowe`, the Labour Code wording, has zero hits — use it only when quoting the statute |
| **employee request** | solicitud | wniosek | запит | The most-spoken product noun on every market — what an employee submits from self-service, before it enters an approval chain. es `solicitud`, pl `wniosek`, uk `запит` |
...
| **absence policy** | política de licencias (the product label) / política de vacaciones (spoken register, and copy where the policy really is about holiday) | polityka nieobecności | політика відсутностей |  |
| **timesheet** | registro de horas | karta pracy | табель обліку робочого часу | **es:** From the corpus: the filled record, distinct from *time tracking* the capability. es `registro de horas` 9 hits and product-confirmed (`application-es-ES.yml:1375-1377`, `:1390`), pl `karta pracy`, uk `табель обліку робочого часу` — **uk takes the full form always, by owner decision; see that row before shortening it** · **uk:** OWNER: **the full form only.** This overrides the product, which shortens to `Табель` in buttons and mail subjects (`application-uk-UA.yml:5`, `:1433`, `mailers-uk-UA.yml:463-481`) and uses the full form where the object is named (`:1470`, `b2b-uk-UA.yml:260`). Bare `табель` is not wrong on a screen, but it is not what we write — do not reintroduce it as a short variant, and do not treat the product's buttons as licence for it |
...
| **performance score** | puntuación de desempeño 🚩 | wskaźnik efektywności | показник ефективності (copy) / Performance Score (latin, only where the line labels the product toggle) | **es:** The composite number, not the review event. pl `wskaźnik efektywności` is PeopleForce's own indicator and rep-attested. Mind the collision that creates: `оцінка ефективності` (28 hits/15 articles) is this glossary's uk form for *performance review*, so the score is `показник` and the review is `оцінка`; using `оцінка` for both is the defect. `рейтинг` is taken by candidate ranking, and `тотал-скоринг` / `середня зважена` are call register · **uk:** From the product locale: es and pl translated this and **uk did not** — the uk product leaves the latin term in the toggle (`use_score: Використовувати Performance Score`, `b2b-uk-UA.yml:552`) and paraphrases elsewhere as `загальна ефективність` (`:553`, `:556`) or `оцінювання ефективності` (`:547`), which collides with *performance review*. es `puntuación de desempeño` (`b2b-es-ES.yml:544-555`, `errors-es-ES.yml:415`) and pl `Wskaźnik Efektywności` (`b2b-pl-PL.yml:549-573`) are settled product strings. So write `показник ефективності` in copy, and if the line labels that toggle, keep it latin |
| **competency gap** | brecha de habilidades (the organisation-level skills gap, as our articles use it) / brecha (the 360 indicator, following the product's severity grades) | luka kompetencyjna / luka kompetencji / luka (the 360 indicator, following the product's severity grades) | розрив / GAP (latin, and only where the line labels the formula the product labels `GAP` — reps voice this as `геп`, which is not a Ukrainian word and must not be written in Cyrillic) | **Two senses; the product names only one.** The 360 review's per-competency delta is `gap_explanation_html` — the formula label stays latin `GAP` in every locale and only the severity grades translate: es `brecha crítica / moderada / insignificante`, pl `luka krytyczna / umiarkowana / nieistotna`, uk `критичний / помірний / незначний розрив` (`application-{es-ES,pl-PL,uk-UA}.yml:5142-5147 / :5295-5301 / :5294-5300`). The organisation-level skills gap is a separate, copy-only sense: es `brecha de habilidades` 8/7 — **habilidades, not competencias** (zero attestation) — pl `luka kompetencyjna` 13 calls. No `ГЕП` cell: `геп` is a rep voicing the latin label |
| **career development plan** | plan de desarrollo | plan rozwoju (the product label) / indywidualny plan rozwoju zawodowego (formal HR register, where the full name of the instrument is the point) | план розвитку | **es:** From the product locale: all three ship the short form — es `plan de desarrollo` (`mailers-es-ES.yml:624-628`), pl `plan rozwoju` (`mailers-pl-PL.yml:621`, `:625`), uk `план розвитку` (`mailers-uk-UA.yml:627-631`). pl `indywidualny plan rozwoju zawodowego` is formal HR register from a 1,200-seat call: correct, but longer than any screen, so use it only where the full name of the instrument is the point · **pl:** From the product locale: all three ship the short form — pl `plan rozwoju` (`mailers-pl-PL.yml:621`, `:625`), es `plan de desarrollo` (`mailers-es-ES.yml:624-628`), uk `план розвитку` (`mailers-uk-UA.yml:627-631`). `indywidualny plan rozwoju zawodowego` is HR-department register from a 1,200-seat call — correct, but longer than any screen |
...
| **payroll deduction** | — | potrącenie | утримання | Three senses, none of them payroll. So ES has no attested form for this concept — ask before choosing, and never reach for `retención`. uk `утримання` is product-derived (`Коригування — утримання`, `application-uk-UA.yml:2288`); pl `potrącenie (z wypłaty)` is corpus-only |
...
| **transparency** | transparencia 🚩 | transparentność | прозорість 🚩 | **es:** pl: *przejrzystość* for transparency as a value; **`jawność wynagrodzeń` once the subject is disclosing pay** — the standard term around Directive (EU) 2023/970. 🚩 loan-versus-native register call pending · **pl:** Owner ruling, and this REVERSES the previous cell, which made przejrzystość the default. Evidence agrees with the reversal: our own pl copy runs transparentno- 266 against przejrzysto- 69, and across 3,401 spoken Polish turns przejrzystość has ZERO hits while the loan runs 7r/7p with reps and prospects using it equally, so the house-dialect objection does not hold. For the pay directive see the EU Pay Transparency Directive row, which takes a different word. |
...
| **spreadsheet** | hoja de cálculo (the default — published copy, and any non-regional register) / planilla (Río de la Plata copy and spoken register — never bare where `planilla de horas` (timesheet) could be read instead) | arkusz kalkulacyjny / arkusze (once the surrounding copy has already established that a spreadsheet is meant) / spreadsheet (declined per Polish grammar: spreadsheeta, spreadsheetow, w spreadsheecie, and never with an apostrophe) / w Excelu / excele (conversational register only) 🚩 | таблиця | **es:** **`hoja de cálculo` HEADS the row** (decided 2026-08-11 — we publish far more than we speak, so the written form takes the default), with `planilla` conditioned on Río de la Plata copy and speech. `planilla` carries a second sense (`planilla de horas` = timesheet, `planillas de sueldo` = pay sheets), so qualify it where either reading is possible · **pl:** The head form itself has never been confirmed by a native, so the flag stays on it. The two variants below ARE owner-confirmed. |
| **self-service** | autogestión / autoservicio 🚩 | samoobsługa 🚩 | самообслуговування 🚩 | Both are correct; prefer `autogestión`. pl `samoobsługa` stands for copy even though reps say the English `self-service` on calls (25 hits against 5) — that borrowing is call register |
...
| **audit trail** | historial de cambios / registro de auditoría (prose about compliance or evidence, never as the product's label) | historia zmian | історія змін | So the product frames this as *history of changes* per object, not as an audit log — `registro de auditoría` is call register only (half of the 30 ES hits) and belongs in prose about compliance. uk `історія змін` is corpus-only: the uk locale has no such string |
...
| **mandate contract** | 🚩 NO ATTESTED FORM | umowa zlecenie | цивільно-правовий договір | **en:** the established English rendering of Polish `umowa zlecenie`. Gloss on first use — it means nothing to a reader without one. Per the MR-328 hr-leaders note · **uk:** the real Ukrainian umbrella category for non-employment work contracts (Civil Code) — the closest true institutional match to Polish `umowa zlecenie`. **Zero corpus attestation** (0 hits for it, `ЦПД`, `договір підряду`, `договір про надання послуг`) because our Ukrainian corpus never discusses these; recorded as a legal-category match, not a corpus finding. In running copy: `людина за цивільно-правовим договором` |
...
| **whistleblowing channel** | canal de denuncias | narzędzie dla sygnalistów | — | **The feature has a name and it is `Safe Speak`** — on the do-not-translate list, confirmed in the pl locale where the surrounding prose is translated but the name is not (`application-pl-PL.yml:9992-9997`). No uk form: the UA market does not raise it |
