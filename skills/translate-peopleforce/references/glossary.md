...
| **workforce** | fuerza laboral | pracownicy | працівники | OWNER RULE 2026-08-11, and the note it replaces is stale — pl and uk were both settled on 2026-08-10 (see those cells), so es was the only open half. **The primary answer is not a noun: restructure.** `workforce` bare almost always paraphrases in marketing prose — *your workforce* → `tu equipo` / `twoi ludzie` / `ваші люди` — and in compounds take the attested compound (`workforce planning` → `planificación de plantilla` / `planowanie zatrudnienia` / `планування штату`). That resolves the es collision instead of relocating it. **Why no other noun works:** the live word `personal` (1,874 hits/130 docs) is already the `staff`/es cell, so taking it here rebuilds the collapse uk just broke. `plantilla` looks attractive at 184/33 and was **rejected** — checked in context, 35 of 38 windows are *template* (`plantillas de documentos`, `plantillas de entrevistas`, `plantillas de correos`), so it collides with the product's own template feature, which is worse than the collision it would fix. That leaves `fuerza laboral`: 123 hits/18 docs in writing, effectively dead in speech (3r/0p), so it is the written-only fallback rather than the head form |
...
| **people operations** | People Operations | People Operations | People Operations | REFLAGGED 2026-08-11 from `contested` (empty) to keep-source. The three cells were empty because there is no attested target form — es 0 hits in 11,899 turns, uk 0 hits — but "no translation" and "no decision" are different states and the empty cell was reporting the wrong one. `People Operations` is a house-style rename of HR that functions as a proper noun, so it is kept in source form the way a product or module name is. This is NOT a coinage: it adds no target-language word. In running prose, describe the function instead (`recursos humanos`, and the specific activity where the sentence allows) rather than calquing the phrase. The owner's 2026-08-10 decline to rule stands on the *target wording*; this records only that there is none to find |
...
| **screening** | selección / preselección (the narrowing pass specifically — filtering a candidate pool down before interviews) 🚩 | screening | скринінг / відбір (where the word has to inflect or agree, and in formal register) | Choosing among candidates. Distinct from recruitment. CORPUS 2026-08-11: 1,451 hits/91 docs, but most of those are `proceso de selección` = the recruitment process, so the count does not confirm the *screening* sense — the head form stays `assumed` and a native still owns it. `preselección` (77/14) is added as a conditioned cell for the narrowing pass specifically — filtering a pool down before interviews — and **not** as a replacement for `selección`. `cribado` was considered and rejected at 3 hits/1 doc. uk: **`скринінг` (84/11) and `відбір` (39/9) are both house forms, context picks** (owner 2026-08-11); prefer `відбір` where the word has to inflect or the register is formal |
...
| **performance** | rendimiento (the practice and the lifecycle stage) / desempeño (reviews and evaluations) / Performance (the product module name) | efektywność | ефективність / Продуктивність (the product module and section name) / результативність (attainment measured against a target, and the name of the discipline in `управління результативністю`) | pl/uk also render *efficiency*, and pl drops the lexeme at *performance review* — which sense each lexeme covers is open-flags 8b. **uk CLOSED 2026-08-11 on the call corpus, and it is a three-way split, not a two-way one:** `ефективність` is the concept (`показники ефективності`, `ефективність співробітників`), `Продуктивність` is the product's module and section name — what prospects say because they are reading the screen — and `результативність` is attainment against a target, giving `управління результативністю` for *performance management*. `Ефективність` at uk-UA 554/1115 against `Продуктивність` at 2242/3435 was never a contradiction; it is a label beside a concept, exactly as es keeps `rendimiento` beside `Performance`. **Never let prose inherit the module label.** This makes pl and uk deliberately diverge on the screen name (pl `efektywność`), which is intended. 🚩 **One caveat the newly-wired divergence alarm surfaced, and it is not a reason to reverse any of the above:** across 70 uk calls reps lead with `продуктивність` (82r/13p) while **customers lead with the bare English `performance`** (26r/14p) — more than any Ukrainian form on the prospect side. Under the alarm's own doctrine the prospect side outranks the rep side for published copy, so this is evidence the module label is not landing in speech. It does not change which Ukrainian word goes where: publishing latin `performance` in Ukrainian copy is a `uk.md` §4 borrowing question, decided on written document spread, not on call frequency. Worth a native's view on the label itself |
...
| **performance review** | evaluación del desempeño 🚩 | ocena pracownicza / ocena okresowa (where the cycle's periodicity is the point (annual, half-yearly)) / ocena efektywności (where the subject is measured performance rather than the review event) | оцінка ефективності / оцінка (second mention, headings, chips and tight layouts — `цикл оцінки`) / перформанс-ревю (spoken register only — quotes and interviews, never published copy) | uk: no house form established — our copy mostly keeps the English or uses *оцінювання*; the cell is a candidate, not settled. pl: *ocena pracownicza* and *okresowa ocena pracownika* are equally standard; bare *ocena* is the tight-layout candidate. **uk SETTLED 2026-08-11, and the short form is the point.** Full form `оцінка ефективності` (68 hits with inflections, against 21 for `оцінювання ефективності`); short form **`оцінка`** on second mention, in headings and in tight layouts (`цикл оцінки`). The two share the head noun, so the short form is a true ellipsis. **`оцінювання` does NOT work as the short form** — the owner rejected it and the corpus says why: it is a different head and a wider concept, used for candidate assessment as much as performance (`критерії оцінювання`, `платформи оцінювання`, `оцінювання навичок`), so dropping the complement changes the referent to assessment-in-general instead of shortening the term. `перформанс-ревю` is attested in speech (44r/8p) and stays spoken. Rule in uk.md §9 |
...
| **feedback** | feedback / retroalimentación (formal and legal register, where a borrowing would read out of place) / comentarios (literal survey comments — a different concept) | feedback | зворотний зв’язок | OWNER 2026-08-11: **keep both forms and favour `feedback`.** It is the head form now, not a conditioned variant — the borrowing is fully current in LatAm HR speech and reads natural in prose (`dar feedback`, `feedback de desempeño`). `retroalimentación` is kept below rather than retired, so nothing published has to change |
| **retention** | retención | retencja | утримання | CORPUS 2026-08-11: 677 hits/74 docs, the strongest of the three. Do not confuse with `rotación` (457/63), which is the **inverse** concept — turnover, not retention — and is frequent enough to be picked up by mistake |
| **turnover** | rotación | rotacja | плинність кадрів | uk: the full phrase, not кадрів alone |
| **absenteeism** | ausentismo 🚩 | absencja | абсентеїзм / прогул (where the word must inflect, and wherever plain register is wanted) 🚩 | es: ausentismo, not the Peninsular absentismo. uk: **`абсентеїзм` and `прогул` are both usable, context picks** — reviewed and kept 2026-08-11 after the count was challenged. The evidence is worth knowing: of 119 hits, **110 sit in a single explainer article** which glosses the word (`Розберемо, що таке абсентеїзм`), and three other documents repeat one boilerplate bullet, against `прогул` at 69 hits across 12 documents. So `абсентеїзм` is a term of art, not everyday vocabulary — which is the right register for professional HR copy, so **gloss it on first mention**. The senses also differ: `прогул` is one unexcused absence, `абсентеїзм` the pattern or metric. This case produced `uk.md` §4 test 3 |
| **absence** | ausencia | nieobecność | відсутність |  |
| **attendance** | asistencia | obecność | відвідуваність |  |
| **time off** | tiempo libre (the request and the notifications around it) / ausencia (the record, the policy and the calendar view) / licencia (the entitlement in running prose — see the leave row) | nieobecność (the record, the policy and the calendar view) / wolne (the request, and the thing an employee takes — `wziąć wolne`, `wniosek o wolne`) 🚩 | відсутність (the record, the policy and the calendar view) / вихідний (the day itself, and the request for one — `взяти вихідний`) 🚩 | SETTLED 2026-08-11 against the live es-ES locale, and it **reverses this note's earlier claim that `tiempo libre` is wrong**. The product itself says it: `request_time_off: Solicitar tiempo libre` (`application-es-ES.yml:1376`, `:1383`, `:10925`), `Tiempo libre aprobado` / `rechazado` / `retirado` / `Detalles de tiempo libre` (`:3455-3479`, `:3767`), plus the attribute help text (`attributes-es-ES.yml:455`, `:1827-1831`). Our own copy agrees — 37 hits/13 published articles, 20/13 in calls, `solicitar tiempo libre` 9 times. The three forms are not competing: **`tiempo libre` is the request**, `ausencia` is the record, policy and calendar sense (`Ausencias de hoy`, `Próxima ausencia`, `política de ausencias`), and `permiso` is the leave-type family (`cualquier otro tipo de permiso`, `application-es-ES.yml:2861`). **pl and uk now carry the same role split, added 2026-08-11.** es had three roles and pl/uk had one cell each, so a translator could not see which role the cell covered. The record, policy and calendar role is `nieobecność` / `відсутність`; the request-and-take role is `wolne` / `вихідний`. 🚩 **The defect this prevents: you take `wolne` or `urlop`, never a `nieobecność`** — `wziąć nieobecność` and `взяти відсутність` are both wrong. The full prose construction map (which verb takes which noun, across all three) still needs a native; only the role split is recorded here |
...
| **payroll** | nómina | płace | нарахування заробітної плати | RE-RULED 2026-08-11. `nómina` is the default and it does **not** route through `liquidación de sueldos`, which three oracles show dead. `planilla` was the proposed CA/PE conditioned variant and it is **held back with a 🚩**, because it now carries three senses across this corpus and would be the most overloaded word in the glossary: *template* in written copy (35 of 38 sampled windows — see the `workforce`/es note), *spreadsheet* in Río de la Plata speech (the `spreadsheet`/es rows), and *payroll* in CA/PE. Our corpus is AR/UY-facing, so the payroll sense is the one we have the least evidence for. Do not add it as a payroll variant until a CA/PE-facing piece actually needs it, and then qualify it. Prior note follows — es regional split. CALL-CORPUS 2026-08-11 adds a second hazard: of 264 `nómina` hits, 14 use it for the employee roster, not the payroll function (`dentro de la nómina yo me voy a un legajo`, `la nómina que van a tener cargada`). Where the source means the list of people, say `directorio` or `listado de colaboradores`; keep `nómina` for pay |
...
| **relocation** | reubicación | relokacja | релокейт / переїзд (the physical move itself, and wherever the word has to inflect) | uk: **both `релокейт` and `переїзд` are house forms and the context picks** (owner 2026-08-11 on loan-versus-native pairs). Corpus: `релокейт` 110 hits/12 docs, `переїзд` 30/8. Check the sense before swapping — `переїзд` is the physical move, `релокейт` the employment arrangement around it. `релокація` was proposed and **rejected**: 5 hits/2 docs, effectively absent |
| **recognition** | reconocimiento | kudosy | кудоси / подяка | CALL-CORPUS 2026-08-11: es `reconocimiento` 252 hits/63 calls. pl and uk name the product feature with the loanword instead and inflect it — `kudosy` 60/38, `кудоси` — while `podziękowanie` / `подяка` are the prose forms |
...
| **time tracking** | seguimiento del tiempo | rejestracja czasu pracy (the product tab, and any line about employees logging their own hours) / śledzenie czasu pracy (where the source is about monitoring rather than the product tab) | облік робочого часу | CALL-CORPUS 2026-08-11: `rejestracja czasu pracy` 19 hits/16 calls against `śledzenie czasu pracy` 7 — and reps use `rejestracja` for the feature because `śledzenie` reads as surveillance of the employee. Keep `śledzenie` only where the source really is about monitoring |
...
| **workflow** | flujo de trabajo | proces | робочий процес / воркфлоу (interchangeable with `робочий процес` — but not where the word has to agree or inflect) | UPDATED 2026-08-11: all three now translate — es `flujo de trabajo`, pl `proces` (uniform in the product locale), uk `робочий процес` after the loan was reversed on document spread. **Still never generalise across languages**: the uk reversal was measured on uk data and reached the same answer pl did by a different route, which is not the same as inheriting it. Speech in both es and uk prefers the English word (es `workflow` 137r/32p against `flujo de trabajo` 121r/6p) — that is register, and it does not reach print. **uk carries both forms as equals** (owner 2026-08-11): `робочий процес` is the head, `воркфлоу` is equally usable, and the context picks. The one place the loan loses is structural rather than stylistic — it is indeclinable, so a sentence needing agreement produces `воркфлоу, що стоїть за ним`, which is what three of four source-blind native passes flagged. Prefer the Ukrainian form there. Short form: bare `процес` once the context has fixed which process. The written corpus favours it on spread (`робочий процес` 300 hits/57 docs against 240/25), which is the oracle the earlier deadlock never counted |
| **AI** | IA | AI | ШІ / AI |  |
...
| **Work type** | Tipo de trabajo | Forma współpracy | Тип роботи | PRODUCT-DERIVED, authoritative: the `employment_type` / `work_type` field label, `config/locales/attributes.yml:45-47`. Read off the app source 2026-08-10, not inferred |
| **Contractor** | Contratista | Contractor 🚩 | підрядник | PRODUCT-DERIVED: one of only three seeded `employment_types` — Full-time, Part-time, Contractor (`app/services/seeds/data.yml.erb:211-232`). **There is no B2B value in the product** |
| **sole trader** |  |  | ФОП | CORPUS-DERIVED: 9 corpus hits, used exactly for companies contracting with sole traders. The closest real Ukrainian counterpart to the Polish `JDG` behind a B2B arrangement — but an institution in its own right, so name it only where the sole-trader status is the point, not as a translation of `B2B` |
...
| **B2B contractor** | 🚩 NO ATTESTED FORM |  | підрядник | OWNER 2026-08-11: **`підрядник` is the default here too**, same as the plain `contractor` row — add `на B2B` only where the arrangement itself is the point, and say `ФОП` where the sole-trader status is what the sentence is about. Earlier note: `B2B-контрактор` was a coinage AND carried the anglicism, twice wrong. That form keeps `B2B` as the acronym it is (a Polish arrangement, not a Ukrainian legal form) and uses the attested noun for the person. Where the sole-trader status is the actual point, say `ФОП` instead |
| **knowledge base** | base de conocimientos | baza wiedzy | база знань | CALL-CORPUS 2026-08-11, corrected to the product's own plural the same day: the es-ES locale says `base de conocimientos` throughout (`attributes-es-ES.yml:2425-2430`), not the singular reps use in speech. The tenant's own article space. pl `baza wiedzy` 27/24 and `attributes` agrees, uk `База знань` (`attributes-uk-UA.yml:444`). Reps also offer `biblioteca`, which is not a product string |
| **help centre** | centro de ayuda | centrum pomocy 🚩 | довідковий центр | PRODUCT-DERIVED 2026-08-11, same key both locales: `help_center` → es `Centro de ayuda` (`application-es-ES.yml:3263`, and in body copy at `:3056`), uk `Довідковий центр` (`application-uk-UA.yml:3400`). PeopleForce's own public docs, **not** the tenant's knowledge base — the two must not be merged. The pl cell stays weak: reps collapse both into `baza wiedzy`, `centrum pomocy` appears once in speech and not at all in the locale |
| **employee directory** | directorio | katalog pracownika | директорія (the product screen) / довідник (a reference list inside the employee card, e.g. `довідник фізичних осіб`) 🚩 | CALL-CORPUS 2026-08-11: es `directorio` 152/82, pl `katalog pracownika` 22/15. uk is split and unsettled — `директорія` 12 for the screen against `довідник` 9 for a reference list; hold one per piece |
| **merge field** | campos variables | pola zmienne | замінники | The placeholder a document template fills from the employee card. **uk is the only product-confirmed cell:** `Як використовувати замінники в шаблоні?` (`application-uk-UA.yml:9666`), checked 2026-08-11. es `campos variables` 15 hits/12 calls and pl `pola zmienne` are corpus-only — the locale files carry neither, so a native could still overrule them. Never transliterate `merge field` |
| **audit trail** | historial de cambios / registro de auditoría (prose about compliance or evidence, never as the product's label) | historia zmian | історія змін | PRODUCT-DERIVED for es and pl 2026-08-11: es `historial de cambios` runs through the locale (`b2b-es-ES.yml:1034`, `:1049`, `application-es-ES.yml:7035`, `:7297`, `:7363`, `:7428`), pl `Historia zmian` likewise (`b2b-pl-PL.yml:1113`, `application-pl-PL.yml:7302`, `:7476`, `:7605`). So the product frames this as *history of changes* per object, not as an audit log — `registro de auditoría` is call register only (half of the 30 ES hits) and belongs in prose about compliance. uk `історія змін` is corpus-only: the uk locale has no such string |
| **geolocation** | geolocalización / geofencing |  |  | CALL-CORPUS 2026-08-11: es only, 1 call but explicit — and it is a partner capability (Clock), not ours. Reps say so outright, so never let copy imply PeopleForce does geofencing. Zero pl and uk attestation |
...
| **qualified electronic signature** | firma electrónica avanzada | kwalifikowany podpis elektroniczny / podpis kwalifikowany | КЕП | PRODUCT-DERIVED 2026-08-11 and the two locales do not even use the same key: es labels it `advanced` → `Firma Electrónica Avanzada` (`attributes-es-ES.yml:646`), uk labels it `qualified` → `КЕП` (`attributes-uk-UA.yml:653`, plus `Український КЕП` at `application-uk-UA.yml:3090-3091` and `:5786`). So the tier is named for what each market's law recognises, which is why this is a separate concept from *electronic signature* — and why ES reps refuse the label `firma digital`. pl `podpis kwalifikowany` is corpus-only here; in the product the pl path is the Autenti integration by name |
| **legal entity** | entidad legal | podmiot prawny | юридична особа | CALL-CORPUS 2026-08-11: es 34/18, uk `юридична особа` 23/9, pl `podmiot prawny` 4. Multi-entity tenants are normal in all three markets and documents are generated per entity, so the term carries weight in product copy |
...
| **job evaluation** | — | wartościowanie stanowisk | — | CALL-CORPUS 2026-08-11: pl 24 hits/14 calls — grading a role to a level and a pay band, the core of PL pay-transparency readiness. Not a *performance review*, and not *job profile*. No ES or UK form: the concept does not come up in those markets |
| **pay gap** | brecha salarial | luka płacowa (copy, and the directive's own vocabulary) / luka wynagrodzeń / różnica wynagrodzeń (quoting or labelling the in-product report) 🚩 | розрив в оплаті | PRODUCT-DERIVED 2026-08-11, and this supersedes both earlier versions of this row: **the product ships a `gender_pay_gap` compliance report in every locale**, so no cell here needed guessing. es `Brecha salarial de género` (`application-es-ES.yml:7144-7145`, `Brecha salarial no ajustada` `:7179`, `Brecha salarial de compensación adicional` `:7191`). uk `Гендерний розрив в оплаті` (`application-uk-UA.yml:7311-7312`, `Нескоригований розрив в оплаті` `:7348`, `:7410-7411`). **The pl product does not say `luka płacowa`** — it wavers between `luka (w) wynagrodzeniach` (`:7324`, `:7349`, `:7361`) and `różnica wynagrodzeń` (`:7325`, `:7411-7414`), so the product is internally inconsistent while our copy, our reps and Polish directive practice all say `luka płacowa`. Keep `luka płacowa` for copy; match the screen when the line names the report |
| **staffing table** | — | — | штатний розпис | CALL-CORPUS 2026-08-11: UA-specific — the approved establishment list a requisition is validated against. Only 2 calls, but structurally load-bearing in the kadrove module. No PL or ES counterpart institution |
| **personnel order** | — | — | наказ | CALL-CORPUS 2026-08-11: UA statutory document pair — the employee files a `заява`, the company issues a `наказ`, and both are generated and signed in-product. No PL or ES equivalent document |
| **military registration** |  |  | військовий облік | CALL-CORPUS 2026-08-11: UA-only employee-card section, a wartime requirement. Never generalise it to another market |
| **work-permit legalisation** |  | legalizacja pracownika |  | CALL-CORPUS 2026-08-11: PL-specific — the residence-and-work-permit trail kept per foreign employee, with expiry dates that trigger workflows |
| **occupational health check** |  | badania okresowe |  | CALL-CORPUS 2026-08-11: PL statutory periodic medical exam; a dated field that drives reminder processes. Do not flatten it to a generic medical check |
| **health and safety training** |  | szkolenie BHP |  | CALL-CORPUS 2026-08-11: PL statutory induction training. `BHP` stays an acronym |
| **contract addendum** |  | aneks do umowy |  | CALL-CORPUS 2026-08-11: PL — what a fixed-term extension or a pay change produces, and the trigger behind the most-cited PL workflow example |
...
| **trial account** | cuenta de prueba | okres testowy / konto demonstracyjne | тестовий акаунт | PRODUCT-CONFIRMED for es 2026-08-11: `su cuenta de prueba de PeopleForce ha terminado` (`mailers-es-ES.yml:428`) — the expiry mail, so this is the term a customer has already seen. The 14–15 day evaluation account every market offers, free and unconditional. pl reps also say `konto demonstracyjne`; the pl and uk cells are corpus-only |
| **subscription** | suscripción | subskrypcja | підписка | CALL-CORPUS 2026-08-11: monthly or annual, and the annual one carries the 20% discount in all three markets |
| **discount** | descuento | zniżka | знижка | CALL-CORPUS 2026-08-11: pl `zniżka` 9 against `rabat` 1 — do not prefer `rabat`. uk `знижка`, es `descuento` |
| **plan** | plan | plan | пакет / тариф | CALL-CORPUS 2026-08-11: the Standard / Professional tier, not a project plan. es and pl keep `plan`; uk reps say `пакет` and `тариф` interchangeably. Module names stay English per the do-not-translate list |
| **quote** | cotización | — | комерційна пропозиція | CALL-CORPUS 2026-08-11: es `cotización` 156 hits/62 calls — the most-repeated commercial noun in the ES corpus. uk `комерційна пропозиція`. pl reps promise a `cennik` instead, so no PL cell for the quote itself |
| **price list** | — | cennik | — | CALL-CORPUS 2026-08-11: PL only. uk `тариф` names the tier, not a list, and ES reps quote (`cotización`) rather than publish one |
