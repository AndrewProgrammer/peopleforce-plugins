...
| **absenteeism** | ausentismo 🚩 | absencja | абсентеїзм / прогул (where the word must inflect, and wherever plain register is wanted) 🚩 | es: ausentismo, not the Peninsular absentismo. uk: **both `абсентеїзм` and `прогул` are house forms, context picks** (owner 2026-08-11) — but the senses differ, `прогул` being the unexcused absence itself and `абсентеїзм` the pattern or metric. 🚩 the loan's 119 hits sit in only **4** documents against 12 for `прогул`, so under uk.md §4's document-spread test it may be one writer's word rather than house usage |
...
| **B2B contractor** | 🚩 NO ATTESTED FORM |  | підрядник на B2B | `B2B-контрактор` was my coinage AND carried the anglicism, twice wrong — replaced 2026-08-10. This form keeps `B2B` as the acronym it is (a Polish arrangement, not a Ukrainian legal form) and uses the attested noun for the person. Where the sole-trader status is the actual point, say `ФОП` instead |
