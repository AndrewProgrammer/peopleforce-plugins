...
| **salary** | salario / sueldo / remuneración | wynagrodzenie | зарплата (marketing, product and prose copy) / заробітна плата (statutory or contractual text) | Three live synonyms, chosen by context. `salario` is the neutral default. `sueldo` is the everyday word for what a person is paid, is what Río de la Plata speech uses, and heads the compounds — `recibo de sueldo`, `planillas de sueldo`. `remuneración` is formal or contractual register. |
