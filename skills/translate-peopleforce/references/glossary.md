...
**Ukrainian copy only** — owner decision. Polish declines names and Spanish never
transliterates, so neither follows from this and both stay open.
...
| **workforce** | fuerza laboral 🚩 | pracownicy | працівники | uk cell emptied by the owner — `персонал` is *staff* and already lives on the staff row. Candidates: `штат` / `кадри` / rephrasing the concept away; native reviewer picks, and says whether marketing copy needs the row at all. pl `kadra` unconfirmed |
...
| **time off** | tiempo libre (the request and the notifications around it) / ausencia (the record, the policy and the calendar view) / licencia (the entitlement in running prose — see the leave row) | nieobecność | відсутність | SETTLED 2026-08-11 against the live es-ES locale, and it **reverses this note's earlier claim that `tiempo libre` is wrong**. The product itself says it: `request_time_off: Solicitar tiempo libre` (`application-es-ES.yml:1376`, `:1383`, `:10925`), `Tiempo libre aprobado` / `rechazado` / `retirado` / `Detalles de tiempo libre` (`:3455-3479`, `:3767`), plus the attribute help text (`attributes-es-ES.yml:455`, `:1827-1831`). Our own copy agrees — 37 hits/13 published articles, 20/13 in calls, `solicitar tiempo libre` 9 times. The three forms are not competing: **`tiempo libre` is the request**, `ausencia` is the record, policy and calendar sense (`Ausencias de hoy`, `Próxima ausencia`, `política de ausencias`), and `permiso` is the leave-type family (`cualquier otro tipo de permiso`, `application-es-ES.yml:2861`) |
...
| **payroll** | nómina 🚩 | płace | нарахування заробітної плати | es regional split. CALL-CORPUS 2026-08-11 adds a second hazard: of 264 `nómina` hits, 14 use it for the employee roster, not the payroll function (`dentro de la nómina yo me voy a un legajo`, `la nómina que van a tener cargada`). Where the source means the list of people, say `directorio` or `listado de colaboradores`; keep `nómina` for pay |
...
| **employee file** | legajo 🚩 | teczka pracownika | картка співробітника | es: legajo is the market term but the product says perfil del empleado. Confirm before customer-facing use — though 419 hits/87 calls make it the strongest ES term in this corpus. **uk is `картка співробітника`, not `профіль`**: the two are different product objects and this cell used to repeat the *employee profile* string, collapsing them. Reps say `картка` 59 times against 4 (corrected 2026-08-11) |
...
| **payroll run** | liquidación | lista płac / naliczanie wynagrodzeń | цикл розрахунку 🚩 | CALL-CORPUS 2026-08-11: the periodic calculation, not *payroll* the function. pl `lista płac` 4 / `naliczanie wynagrodzeń`. The uk cell is weak: reps mostly borrow (`payroll-ран`, `GROSS-цикл`) and say `цикл розрахунку` only in passing |
