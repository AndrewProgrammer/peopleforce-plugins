...
| **approver** | aprobador | zatwierdzający | схвалювач | uk `погоджувач` also occurs — hold one form per piece. pl `osoba zatwierdzająca` is a live variant |
...
| **HR management** | gestión de RRHH |  | HR-менеджмент | **es:** HR management · **uk:** Acceptable in general use when the sense is the concept; does not displace `управління персоналом` as the head form. |
...
| **absenteeism** | ausentismo 🚩 | absencja | абсентеїзм 🚩 | es: ausentismo, not the Peninsular absentismo. uk: **both `абсентеїзм` and `прогул` are usable, context picks** — `абсентеїзм` is a term of art (**gloss it on first mention**) and names the pattern or metric; `прогул` is one unexcused absence |
| **absence** |  | nieobecność | відсутність |  |
...
| **employee benefits** | beneficios 🚩 | świadczenia / benefity | бенефіти / пільги | **es:** pl: świadczenia, not korzyści, which means advantages · **pl:** `świadczenia` leans formal or statutory; `benefity` leans to the package a company advertises. Pick by which the sentence is doing, and hold it through the piece. · **uk:** No signal either way — pick one and hold it across a piece. 🚩 the product's own uk string for this screen is unverified |
...
| **probation period** | periodo de prueba | okres próbny | випробувальний термін | employee profile |
| **employee profile** |  | profil pracownika | профіль співробітника |  |
...
| **career development plan** | plan de desarrollo | plan rozwoju | план розвитку | All three ship the short form. pl `indywidualny plan rozwoju zawodowego` is formal HR register — use it only where the full name of the instrument is the point. |
...
| **HR administration** | — | kadry | кадрове адміністрування / кадрові процеси | The statutory personnel-records layer — orders, applications, the P-2 card — a separate module in UA. pl calls the function `kadry`, `kadry i płace` when paired with payroll. es has no counterpart layer |
...
| **transparency** | transparencia 🚩 | transparentność | прозорість 🚩 | pl: **`jawność wynagrodzeń` once the subject is disclosing pay** — the standard term around Directive (EU) 2023/970; see the directive row |
| **wellbeing** | bienestar 🚩 | dobrostan 🚩 | добробут | **es:** All three settle the same way: **the native form and the English loan are both house forms and the context picks** — pl `dobrostan` for the programme or the discipline, the loan where the source says *well-being*; uk `добробут`. **`samopoczucie` and `самопочуття` are a different concept** — how a person feels on the day — keep them out of this slot · **uk:** `добробут` is the word for the HR sense (`програми добробуту`). `благополуччя` is calque-flavoured — **deprecated rather than wrong**, leave it where already published. `самопочуття` is a **different concept** — how a person feels on the day — do not use it here |
...
| **spreadsheet** | hoja de cálculo (the default, now unconditional for published copy) / planilla (Southern-Cone spoken register only — never bare where `planilla de horas` (timesheet) could be read instead) | arkusze (once the surrounding copy has already established that a spreadsheet is meant) / spreadsheet (declined per Polish grammar: spreadsheeta, spreadsheetow, w spreadsheecie, and never with an apostrophe) / w Excelu / excele (conversational register only) 🚩 |  | **`hoja de cálculo` heads the row** — the written form takes the default. `planilla` survives only as Southern-Cone spoken register, and carries a second sense (`planilla de horas` is a timesheet) — never leave it bare where that could be read. |
...
| **B2B contractor** | — |  | підрядник | **`підрядник` is the default here too**, same as the plain `contractor` row — add `на B2B` only where the arrangement itself is the point, and say `ФОП` where the sole-trader status is what the sentence is about. Never write `B2B-контрактор` — a coinage carrying the anglicism |
...
| **staffing table** | — | — | штатний розпис | Structurally load-bearing in the kadrove module. No PL or ES counterpart institution |
| **personnel order** | — | — | наказ | No PL or ES equivalent document |
| **military registration** | — | — | військовий облік | UA-only employee-card section, a wartime requirement. Never generalise it to another market |
| **work-permit legalisation** | — | legalizacja pracownika | — | PL-only concept — es and uk hold no form |
...
| **contract addendum** | — | aneks do umowy | — | PL-only concept — es and uk hold no form |
...
| **milestone** | hito | krok milowy (quoting a source that already says it — the Autenti case-study wording) / — | великий крок уперед | the figurative sense: a significant step forward. pl source is usually *krok milowy*. For a dated project checkpoint this is a different concept — see the *checkpoint* rows |
...
| **price list** | — | cennik | — | PL only. uk `тариф` names the tier, not a list; ES quotes (`cotización`) rather than publishes one |
